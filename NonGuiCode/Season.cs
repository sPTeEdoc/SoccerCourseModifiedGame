...
namespace FunnyOldGameRedux
{
    public class Season
    {
        ...
        public DateTime startOfSeason = new DateTime(DateTime.Now.Year, 8, 1);
        ...
        public DateTime endTheSeason = new DateTime(DateTime.Now.Year, 7, 31);
        ...
        public Dictionary<string, string> leaguesAbove = new Dictionary<string, string>();
        public Dictionary<string, string> leaguesBelow = new Dictionary<string, string>();

        public long TimeToSim { get; set; }
        ...
        public Season()
        {
            ...
            //Die d4 = new Die(4);
            internationalBreakWeeks.Add(9, 3); // september
            internationalBreakWeeks.Add(10, 3); // october
            internationalBreakWeeks.Add(11, 3); // november
            internationalBreakWeeks.Add(12, 4); // december
            internationalBreakWeeks.Add(3, 3); // march

            endTheSeason = new DateTime(startOfSeason.Year + 1, 7, 31);
            ...
            foreach (LeagueCountry c in TeamRepository.Instance.countries)
            {
                List<League> leagues = TeamRepository.Instance.countries[TeamRepository.Instance.countryIndex[c.CountryName]].leagues;
                for (int i = 0; i < leagues.Count; i++)
                {
                    List<Team> teams = leagues[i].teams;
                    foreach (Team team in teams)
                    {
                        foreach (Player p in team.completeRoster)
                        {
                            ...
                            if (leagues[i].LeagueName != "UEFA Champions League" &&
                                leagues[i].LeagueName != "UEFA Europa League")
        ...
        {
            teamRank = new Dictionary<Team, int>();
            ranks = new List<int>();
            teamRanksOutput = new string[4];
            TopTeamEPL = "";
            cupGames = new Dictionary<string, List<List<Game>>>();
            countryCupRoundOneTeams = new Dictionary<string, List<Team>>();
            countryCupRoundTwoTeams = new Dictionary<string, List<Team>>();
            cupRoundByCountry = new Dictionary<string, int>();
            earlierGame = DateTime.MaxValue;
            latestGame = DateTime.MinValue;
            countryFixtures = new Dictionary<string, Dictionary<DateTime, List<Game>>>();
            leagueFixtures = new Dictionary<string, Dictionary<DateTime, List<Game>>>();
            countriesLeagueMatchesScheduledOnDay = new Dictionary<DateTime, List<string>>();
            countriesCupMatchesScheduledOnDay = new Dictionary<DateTime, List<string>>();
            cupFixtures = new Dictionary<string, Dictionary<DateTime, List<Game>>>();

            promotionPlayoffs = new List<Dictionary<DateTime, Dictionary<string, List<Game>>>>();

            gameLeggedPairs = new Dictionary<Game, Game>();
            teamRankInLeague = new Dictionary<string, Dictionary<Team, int>>();

            seasonGameDate = DateTime.MaxValue;
            viewableGameDate = DateTime.MaxValue;
            startOfSeasonDate = DateTime.MaxValue;
            endOfSeasonDate = DateTime.MinValue;

            m_UefaChampion = null;
            m_EuropaChampion = null;
            m_TrebleWinner = null;

            cupChampions = new Dictionary<string, Team>();
            promotionalPlayoffWinners = new Dictionary<string, Team>();
            countryLegName = new Dictionary<DateTime, Dictionary<string, string>>();

            leagueChampions = new List<Dictionary<string, Team>>();
            leaguePromotions = new List<Dictionary<string, List<Team>>>();
            leagueDemotions = new List<Dictionary<string, List<Team>>>();

            leaguesAbove = new Dictionary<string, string>();
            leaguesBelow = new Dictionary<string, string>();
        }
                    ...
                    for (int j = 0; j < l.Count; j++)
                    {
                        ...
                        if (l.Count > 0)
                        {
                            if ((j + 1) < l.Count)
                            {
                                ...
                                leaguesBelow.Add(l[j].LeagueName, l[j + 1].LeagueName);
                            ...
                            if (j > 0)
                            {
                                ...
                                leaguesAbove.Add(l[j].LeagueName, l[j - 1].LeagueName);
        ...
        private void ScheduleUEFAMatches(Dictionary<DateTime, List<Game>> allLeagueFixtures, string championshipTpe)
        {
            ...
            if (!leagueDictionary.ContainsKey(UEFAleagues[0].LeagueName))
                leagueDictionary.Add(UEFAleagues[0].LeagueName, UEFATeams);
            else
                leagueDictionary[UEFAleagues[0].LeagueName] = UEFATeams;
        ...
        public string GetStatString(string leagueName, string teamName)
        {
            ...
            SortPlayersLeague(players, false, false, false, false, false, false, false, false, false, true, leagueName,
                teamName);
            teamRankString += "Matches Played:" + Environment.NewLine;
            for (int i = 0; i < 20; i++)
            {
                if (players[i].LeagueTeamSeasonStats[leagueName][teamName].matchesPlayed > 0)
                    teamRankString += players[i].fullName + "(" + (playerTeamMap[players[i]].Name) + "): "
                        + players[i].LeagueTeamSeasonStats[leagueName][teamName].matchesPlayed + Environment.NewLine;
            }

            SortPlayersLeague(players, true, false, false, false, false, false, false, false, false, false, leagueName,
                teamName);
            teamRankString += "Goals leaders:" + Environment.NewLine;
            for (int i = 0; i < 20; i++)
            {
                if (players[i].LeagueTeamSeasonStats[leagueName][teamName].goals > 0)
                    teamRankString += players[i].fullName + "(" + (playerTeamMap[players[i]].Name) + "): "
                        + players[i].LeagueTeamSeasonStats[leagueName][teamName].goals + Environment.NewLine;
            }
            SortPlayersLeague(players, false, false, false, false, false, false, true, false, false, false, leagueName,
                teamName);
            teamRankString += Environment.NewLine;
            teamRankString += "Goals efficiency leaders (minimum ten shots):" + Environment.NewLine;
            for (int i = 0; i < 20; i++)
            {
            ...
            Dictionary<int, List<Team>> championsAndRelegations, string leagueName)
        {
            ...
            DateTime date = startOfSeason;
            teamRankString += date.Year.ToString() + " " + leagueName + " Season: " + Environment.NewLine + Environment.NewLine;
            ...
            foreach (Team team in leagueTeams)
            {
                ...
                teamRankString += rankStringStandings[teamRank[team]].ToString() +
                    ...
                    (team.seasonStats[leagueName].goals - team.seasonStats[leagueName].goalsConceded).ToString() + Environment.NewLine;
        ...
        private void SchedulePromotionPlayOffRound(int round, DateTime newLatestGame)
        {
            ...
            if (round == 1)
            {
                games.Add(new Game(eflChampionshipTeams[5], eflChampionshipTeams[2], 45, false, false, false, 5, currentLatest,
                    "EFL Championship", round, "EFL Championship Playoff Round:",
                    true, false, true));
                games.Add(new Game(eflChampionshipTeams[4], eflChampionshipTeams[3], 45, false, false, false, 5, currentLatest,
                    "EFL Championship", round, "EFL Championship Playoff Round:",
                    true, false, true));
                ...
                games = new List<Game>();
                games.Add(new Game(eflLeagueOneTeams[5], eflLeagueOneTeams[2], 45, false, false, false, 5, currentLatest,
                    "EFL League One", round, "EFL League One Playoff Round:",
                    true, false, true));
                games.Add(new Game(eflLeagueOneTeams[4], eflLeagueOneTeams[3], 45, false, false, false, 5, currentLatest,
                    "EFL League One", round, "EFL League One Playoff Round:",
                    true, false, true));
                leagueRound.Add("EFL League One", games);

                games = new List<Game>();
                games.Add(new Game(eflLeagueTwoTeams[6], eflLeagueTwoTeams[3], 45, false, false, false, 5, currentLatest,
                    "EFL League Two", round, "EFL League Two Playoff Round:",
                    true, false, true));
                games.Add(new Game(eflLeagueTwoTeams[5], eflLeagueTwoTeams[4], 45, false, false, false, 5, currentLatest,
                    "EFL League Two", round, "EFL League Two Playoff Round:",
                    true, false, true));
            ...
            if (round == 2)
            {
                ...
                Game game = new Game(eflChampionshipTeams[2], eflChampionshipTeams[5], 45, false, true, false, 5, currentLatest,
                    "EFL Championship", round, "EFL Championship Playoff Round:",
                    true, false, true);
                ...
                games.Add(game);
                game = new Game(eflChampionshipTeams[3], eflChampionshipTeams[4], 45, false, true, false, 5, currentLatest,
                    "EFL Championship", round, "EFL Championship Playoff Round:",
                    true, false, true);
                ...
                games = new List<Game>();
                game = new Game(eflLeagueOneTeams[2], eflLeagueOneTeams[5], 45, false, true, false, 5, currentLatest,
                    "EFL League One", round, "EFL League One Playoff Round:",
                    true, false, true);
                game.AwayFirstLeg = round1Games[0].HomeScore;
                game.HomeFirstLeg = round1Games[0].AwayScore;
                games.Add(game);
                game = new Game(eflLeagueOneTeams[3], eflLeagueOneTeams[4], 45, false, true, false, 5, currentLatest,
                    "EFL League One", round, "EFL League One Playoff Round:",
                    true, false, true);
                game.AwayFirstLeg = round1Games[1].HomeScore;
                game.HomeFirstLeg = round1Games[1].AwayScore;
                ...
                games = new List<Game>();
                game = new Game(eflLeagueTwoTeams[3], eflLeagueTwoTeams[6], 45, false, true, false, 5, currentLatest,
                    "EFL League Two", round, "EFL League Two Playoff Round:",
                    true, false, true);
                game.AwayFirstLeg = round1Games[0].HomeScore;
                game.HomeFirstLeg = round1Games[0].AwayScore;
                games.Add(game);
                game = new Game(eflLeagueTwoTeams[4], eflLeagueTwoTeams[5], 45, false, true, false, 5, currentLatest,
                    "EFL League Two", round, "EFL League Two Playoff Round:",
                    true, false, true);
                game.AwayFirstLeg = round1Games[1].HomeScore;
                game.HomeFirstLeg = round1Games[1].AwayScore;
            ...
            if (round == 3)
            {
                ...
                Game game = new Game(round1Games[0].Winner, round1Games[1].Winner, 45, false, true, false, 5, currentLatest,
                    "EFL Championship", round, "EFL Championship Playoff Round:",
                    true, false, true);
                games.Add(game);
                leagueRound.Add("EFL Championship", games);
                ...
                games = new List<Game>();
                game = new Game(round1Games[0].Winner, round1Games[1].Winner, 45, false, true, false, 5, currentLatest,
                    "EFL League One", round, "EFL League One Playoff Round:",
                    true, false, true);
                games.Add(game);
                leagueRound.Add("EFL League One", games);
        ...
        public async void SimulateGame(Game g)
        {
            ...
            if (!g.GamePlayed)
            {
                ...
                g.BeginGame(false);
                //MatchManager match = new MatchManager(g);
                //match.CurrentRunMode = MatchManager.MatchRunMode.SimulateOnly;
                //MyLogger.Initialize();
                //Task gameTask = match.StartMatch();
                //while (match.IsMatchRunning)
                //{
                //    await Task.Delay(50);
                //}
                //await gameTask; // Wait for the game simulation task to complete gracefully
                ////Console.WriteLine("Match simulation has concluded.");

                //// Now gracefully shut down the logger using await
                //await MyLogger.Shutdown();
        ...
        public async Task SimulateGames(List<Game> games) // Make this async Task to await the parallel games
        {
            var simulationTasks = new List<Task>();

            foreach (Game g in games)
            {
                // For each game, create a Task that runs the synchronous game logic on a thread pool thread.
                // We'll pass `g` into a lambda to capture it correctly for each task.
                simulationTasks.Add(Task.Run(async () => // Make the lambda async too
                {
                    LowerGamesOut(g.HomeTeam);
                    LowerGamesOut(g.AwayTeam);
                    g.HomeTeam.ResetStats();
                    ...
                    if (!g.GamePlayed)
                    {
                        ...
                        await g.BeginGame(false); // NOW AWAIT BeginGame inside Task.Run
                    }
                }));
            }

            // Await all simulation tasks to complete.
            // This makes SimulateGames asynchronous and efficient.
            await Task.WhenAll(simulationTasks);

            //// After all games are done, you might want to do something here,
            //// e.g., update standings, save results, etc.
            //Console.WriteLine($"All {games.Count} games simulated concurrently!");
        }
        ...
        public void ScheduleEuropeKnockoutRounds(Dictionary<DateTime, List<Game>> CupFixtures, string leagueName)
        {
            ...
            while (gamesScheduled < totalGames)
            {
                ...
                Game game1 = new Game(teamA, teamB, 45, false, false, false, 5, date,
                ...
                Game game2 = new Game(teamB, teamA, 45, false, true, false, 5, date2,
            ...
            int month, bool shuffleTeams, bool scheduleDoubleLeg, bool scheduleEndOfMonth, DayOfWeek day)
        {
            ...
            for (int i = 0; i < teams.Count; i += 2)
            {
                ...
                Game game = new Game(teamA, teamB, 45, false, 
                    (country != "UEFA Champions League" && country != "UEFA Europa League") || cupRound == -1, false, 5,
                ...
                if (scheduleDoubleLeg)
                {
                    ...
                    Game game2 = new Game(teamB, teamA, 45, false, true, false, 5,
            ...
            List<Team> cupRoundOneTeams, DateTime date)
        {
            ...
            for (int i = 0; i < cupRoundOneTeams.Count; i += 2)
            {
                ...
                Game game = new Game(cupRoundOneTeams[i], cupRoundOneTeams[i + 1], 45, false, true, false, 5,
            ...
            string leagueName)
        {
            ...
            for (int week = 0; week < numberOfMatchesToSchedule; week++)
            {
                ...
                if (scheduleSecondTeamFirst)
                    game = new Game(ListTeam[0], teams[teamIdx], 45, false, false, false, 5, date, leagueName,
                        fixtureNumber, leagueDictionary + " Fixture:");
                else
                    game = new Game(teams[teamIdx], ListTeam[0], 45, false, false, false, 5, date, leagueName,
                        fixtureNumber, leagueDictionary + " Fixture:");
                ...
                for (int idx = 1; idx < halfSize; idx++)
                {
                    ...
                    if (scheduleSecondTeamFirst)
                        game = new Game(teams[secondTeam], teams[firstTeam], 45, false, false, false, 5, date, leagueName,
                        fixtureNumber, leagueDictionary + " Fixture:");
                    else
                        game = new Game(teams[firstTeam], teams[secondTeam], 45, false, false, false, 5, date, leagueName,
                        fixtureNumber, leagueDictionary + " Fixture:");
                    if (game.HomeTeam.Name != "Bye Week" && game.AwayTeam.Name != "Bye Week")
                    {
                        games.Add(game);
        ...
        {
            startOfSeason = new DateTime(startOfSeason.Year + 1, 8, 1);
            TeamRepository.Instance.PopulateNewEuroTeams();
            foreach (Player p in playersInLeague)
            {
                p.LeagueTeamSeasonStats = new Dictionary<string, Dictionary<string, GameStats>>();
            }

            for (int i = 0; i < Season.Instance.leagueDemotions.Count; i++)
            {
                foreach (KeyValuePair<string, List<Team>> entry in Season.Instance.leagueDemotions[i])
                {
                    foreach (Team t in entry.Value)
                    {
                        TeamRepository.Instance.leagueNameDict[entry.Key].teams.Remove(t);
                        string leagueBelow = Season.Instance.leaguesBelow[entry.Key];
                        TeamRepository.Instance.leagueNameDict[leagueBelow].teams.Add(t);
                    }
                }
            }
            for (int i = 0; i < Season.Instance.leaguePromotions.Count; i++)
            {
                foreach (KeyValuePair<string, List<Team>> entry in Season.Instance.leaguePromotions[i])
                {
                    foreach (Team t in entry.Value)
                    {
                        TeamRepository.Instance.leagueNameDict[entry.Key].teams.Remove(t);
                        string leagueAbove = Season.Instance.leaguesAbove[entry.Key];
                        TeamRepository.Instance.leagueNameDict[leagueAbove].teams.Add(t);
                    }
                }
            }
            endTheSeason = new DateTime(startOfSeason.Year + 1, 7, 31);
            foreach (League l in TeamRepository.Instance.leagues)
            {
                List<Team> teams = l.teams;
                foreach (Team t in teams)
                {
                    t.seasonStats = new Dictionary<string, GameStats>();
                    t.playerStats = new Dictionary<string, Dictionary<int, GameStats>>();
                    t.ResetStats();
                }
            }
